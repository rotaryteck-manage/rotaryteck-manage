...
function platingChanges(before,after){
 ...
 for(const [key,label]of [['number','送鍍次數'],['vendor','廠商'],['sent','寄出日期'],['returned','回貨日期'],['inspection','品檢'],['note','備註'],['welderName','焊接人員']]){
...
function renderPlating(){
 ...
 $('main').append(section);const ledgerButton=document.createElement('button');ledgerButton.type='button';ledgerButton.className='small';ledgerButton.textContent=pt('ledger');ledgerButton.onclick=openPlatingLedger;section.querySelector('.plating-head').append(ledgerButton);
 if(canExportPhotos()){const b=document.createElement('button');b.type='button';b.className='small';b.textContent=pt('exportAllPhotos');b.onclick=()=>exportPlatingPhotos(platingProjects(),b);section.querySelector('.plating-head').append(b);}
 function draw(query=''){
...
function editPlatingShipment(projectId,shipmentId,copy=false){
 ...
 $('#modal [name=returned]').oninput=e=>{$('#shipment-status').value=pt(e.target.value?'completed':'sending');};loadWelderOptions(s);
...
function attachPlatingPhotoOverview(p){
 const panel=document.createElement('section');panel.className='plating-photo-panel';
 panel.innerHTML=(canExportPhotos()?'<button type="button" class="small" data-project-photo-export>'+pe('exportProjectPhotos')+'</button>':'')+'<details class="receipt-files-fold"><summary>'+pe('photoOverview')+' <span data-photo-count></span></summary><div data-photo-list></div></details><p data-photo-status role="status"></p>';
...
async function exportPlatingPhotos(projects,button,status){
 if(!canExportPhotos()||platingPhotoExportBusy)return;
 ...
 try{
  if(!await authorizeExport())return;show(pt('exportWorking'));const entries=platingRecordEntries(targets,logs),manifest=[['案名','送鍍次數','照片類別','人員','上傳時間','原始檔名','檔案位置']];
  for(const [pi,p]of targets.entries())for(const s of p.shipments){const items=await listPlatingPhotos(p,s,true);
   for(const [i,item]of items.entries()){
    const r=await apiFetch(platingPhotoUrl(p,s)+'&export=1&id='+encodeURIComponent(item.id));if(!r.ok)throw Error('照片下載失敗：'+p.name+' '+platingCount(s));
...
if(typeof window!=='undefined')window.addEventListener('resize',()=>document.querySelector('.plating-workspace .plating-grid')?.updatePlatingControls?.());
