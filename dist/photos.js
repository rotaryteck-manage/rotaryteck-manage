...
function equalizeControls(){if(sizingControls)return;sizingControls=true;requestAnimationFrame(()=>{try{new Set([...document.querySelectorAll('button')].map(button=>button.parentElement)).forEach(group=>{const buttons=[...group.children].filter(el=>el.matches('button:not(.wire-thumb):not(.plating-open):not(#close-modal),a.admin-back-button,a.admin-back-link'));if(buttons.length<2)return;group.removeAttribute('data-equal-controls');group.style.removeProperty('--action-width');const width=Math.ceil(Math.max(...buttons.map(b=>b.getBoundingClientRect().width)));if(width>0){group.setAttribute('data-equal-controls','');group.style.setProperty('--action-width',width+'px');}});}finally{sizingControls=false;}});}
