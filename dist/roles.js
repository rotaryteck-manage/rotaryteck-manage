...
async function loadEmployees(){
 ...
 try{
  ...
  box.innerHTML=employeesCache.map(e=>`<div class="employee-row compact"><div><strong>${esc(e.name)}</strong><small>${esc(e.email)} · ${esc(roleNames[e.role])} · ${e.status==='active'?'啟用':'停用'}${e.account_user_id&&e.last_login_at?' · 最後登入 '+esc(receiptTime(e.last_login_at)):''}</small></div><button type="button" data-edit-employee="${e.id}">修改</button>${e.id===data.currentEmployeeId?'':`<button type="button" class="danger-button" data-delete-employee="${e.id}">刪除</button>`}</div>`).join('')||'<div class="empty">尚未建立員工</div>';
...
function employeeDialog(id){
 ...
 modal(id?'修改員工':'邀請員工',field('員工姓名','name',existing.name,'required maxlength="80"')+field('登入信箱','email',existing.email,'type="email" required maxlength="200"')+'<div class="form-grid"><label class="field">權限層級<select name="role">'+Object.entries(roleNames).map(([v,l])=>'<option value="'+v+'" '+(existing.role===v?'selected':'')+'>'+l+'</option>').join('')+'</select><small id="role-help">'+esc(roleDescriptions[existing.role])+'</small></label><label class="field">帳號狀態<select name="status"><option value="active" '+(existing.status==='active'?'selected':'')+'>啟用</option><option value="disabled" '+(existing.status==='disabled'?'selected':'')+'>停用</option></select></label></div>',id?'儲存員工':'寄送邀請',async fd=>{const payload={id,name:String(fd.get('name')||'').trim(),email:String(fd.get('email')||'').trim(),role:String(fd.get('role')),profileId:String(fd.get('profileId')||''),status:String(fd.get('status'))},r=await apiFetch('/api/employees',{method:id?'PUT':'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}),data=await r.json();if(!r.ok)error(data.error||'員工設定未完成');addAudit(id?'修改員工權限':'邀請員工帳號',payload.name+' · '+roleNames[payload.role],'管理後台 > 員工與權限');persist();$('#modal').close();renderAdmin();toast(id?'員工資料已更新':'邀請信已寄出，員工可自行設定密碼');});const select=$('#modal select[name="role"]');select.onchange=()=>{$('#role-help').textContent=roleDescriptions[select.value];};
}
async function deleteEmployee(id){const e=employeesCache.find(x=>x.id===id);if(!e||!(await confirmAction('確定刪除員工「'+e.name+'」？此員工將失去系統權限。')))return;try{const r=await apiFetch('/api/employees',{method:'DELETE',headers:{'Content-Type':'application/json'},body:JSON.stringify(e)}),data=await r.json();if(!r.ok)throw Error(data.error||'無法刪除員工');addAudit('刪除員工帳號',e.name,'管理後台 > 員工與權限');persist();renderAdmin();toast('員工已刪除');}catch(err){toast(err.message);}}
