...
import {images,loginLogo,appIcon52,appIconSettings55,appIconSource55,appManifest55,appIndexBody55} from '../worker/server.mjs';
...
test('private images, role enforcement, project ownership and persistent receipts',async()=>{const objects=new Map(),env={SUPABASE_URL:'https://test.supabase.co',SUPABASE_PUBLISHABLE_KEY:'publishable',SUPABASE_SECRET_KEY:'secret',DB:database(),UPLOADS:{async put(key,body,meta){objects.set(key,{body,...meta,key,uploaded:new Date()});},async get(key){return objects.get(key);},async head(key){return objects.get(key);},async list({prefix}){return {objects:[...objects.values()].filter(x=>x.key.startsWith(prefix)),truncated:false};},async delete(keys){for(const key of Array.isArray(keys)?keys:[keys])objects.delete(key);}}};
 ...
 iconForm.append('source',new Blob([png],{type:'image/png'}),'original.png');
 iconForm.append('homeName','線材小站');iconForm.append('background','#e8dccc');iconForm.append('removeWhite','false');
 ...
 assert.deepEqual(new Uint8Array(await (await appIconSource55(req('/api/app-icon-source'),env)).arrayBuffer()),png);
 assert.equal((await appIconSource55(req('/api/app-icon-source','GET',undefined,'viewer'),env)).status,403);
 const settings=await (await appIconSettings55(req('/api/app-icon-settings','GET',undefined,''),env)).json();
 assert.equal(settings.homeName,'線材小站');assert.equal(settings.background,'#e8dccc');assert.equal(settings.removeWhite,false);
 const manifest=await (await appManifest55(req('/manifest.webmanifest','GET',undefined,''),env)).json();
 assert.equal(manifest.short_name,'線材小站');assert.equal(manifest.background_color,'#e8dccc');
 assert.match(appIndexBody55('<meta name="apple-mobile-web-app-title" content="擎正管理">',settings),/content="線材小站"/);
 assert.equal(appIndexBody55('<meta name="apple-mobile-web-app-title" content="擎正管理">',{...settings,homeName:'<試>"'}),'<meta name="apple-mobile-web-app-title" content="&lt;試&gt;&quot;">');
 assert.equal((await images(req('/api/logo','POST',png),env)).status,200);
 assert.equal((await (await appIconSettings55(req('/api/app-icon-settings','GET',undefined,''),env)).json()).background,'#e8dccc');
 assert.equal((await images(req('/api/logo'),env)).headers.get('Content-Type'),'image/png');
