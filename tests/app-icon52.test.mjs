...
test('icon writes require a logged in supervisor and handles absent logo',async()=>{assert.equal((await appIcon52(new Request('https://example.com/api/app-icon',{method:'POST'}),{})).status,401);assert.equal((await appIcon52(new Request('https://example.com/api/app-icon'),{})).status,404);});
test('manifest and iPhone metadata reference public logo route',()=>{const m=JSON.parse(fs.readFileSync('dist/manifest.webmanifest','utf8'));assert.equal(m.start_url,'/');assert.equal(m.icons[0].src,'/api/app-icon?v=53');assert.equal(m.short_name,'擎正管理');assert.match(fs.readFileSync('dist/index.html','utf8'),/rel="apple-touch-icon" href="\/api\/app-icon\?v=53"/);});
